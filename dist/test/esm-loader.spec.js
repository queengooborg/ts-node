...
const spawn = (0, exec_helpers_1.createSpawn)({
    cwd: helpers_1.TEST_DIR,
});
test.suite('esm', (test) => {
    test.suite('when node supports loader hooks', (test) => {
        ...
        test('should compile and execute as ESM', async () => {
            const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} index.ts`, {
        ...
        test('should use source maps', async (t) => {
            const { err, stdout, stderr } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} "throw error.ts"`, {
                cwd: (0, path_1.join)(helpers_1.TEST_DIR, './esm'),
            });
        ...
        test.suite('supports experimental-specifier-resolution=node', (test) => {
            test('via --experimental-specifier-resolution', async () => {
                const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} --experimental-specifier-resolution=node index.ts`, { cwd: (0, path_1.join)(helpers_1.TEST_DIR, './esm-node-resolver') });
                expect(err).toBe(null);
                expect(stdout).toBe('foo bar baz biff libfoo\n');
            });
            test('via --es-module-specifier-resolution alias', async () => {
                const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ${helpers_1.EXPERIMENTAL_MODULES_FLAG} --es-module-specifier-resolution=node index.ts`, { cwd: (0, path_1.join)(helpers_1.TEST_DIR, './esm-node-resolver') });
                expect(err).toBe(null);
                expect(stdout).toBe('foo bar baz biff libfoo\n');
            });
            test('via NODE_OPTIONS', async () => {
                const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} index.ts`, {
                    ...
                    env: {
                        ...process.env,
                        NODE_OPTIONS: `${helpers_1.EXPERIMENTAL_MODULES_FLAG} --experimental-specifier-resolution=node`,
                    },
                });
                expect(err).toBe(null);
                expect(stdout).toBe('foo bar baz biff libfoo\n');
        ...
        test('throws ERR_REQUIRE_ESM when attempting to require() an ESM script when ESM loader is enabled', async () => {
            const { err, stderr } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ./index.js`, {
        ...
        test('defers to fallback loaders when URL should not be handled by ts-node', async () => {
            const { err, stdout, stderr } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} index.mjs`, {
        ...
        test('should support transpile only mode via dedicated loader entrypoint', async () => {
            const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT}/transpile-only index.ts`, {
        ...
        test.suite('moduleTypes', (test) => {
            ...
            function suite(name, tsconfig) {
                test.suite(name, (test) => {
                    test('supports CJS webpack.config.ts in an otherwise ESM project', async (t) => {
                        ...
                        const { err, stdout } = await exec(`${helpers_1.CMD_TS_NODE_WITHOUT_PROJECT_FLAG} --project ./module-types/override-to-cjs/${tsconfig} ./module-types/override-to-cjs/test-webpack-config.cjs`);
                        expect(err).toBe(null);
                        expect(stdout).toBe(``);
                    });
            ...
            async function run(project, config, ext) {
                const { err, stderr, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ./module-types/${project}/test.${ext}`, {
                    env: {
                        ...process.env,
        ...
        test.suite('createEsmHooks()', (test) => {
            test('should create proper hooks with provided instance', async () => {
                const { err } = await exec(`node ${helpers_1.EXPERIMENTAL_MODULES_FLAG} --loader ./loader.mjs index.ts`, {
        ...
        test.suite('unit test hooks', ({ context }) => {
            ...
            test.suite('data URIs', (test) => {
                test.runIf(helpers_1.nodeUsesNewHooksApi);
        ...
        test.suite('supports import assertions', (test) => {
            test.runIf(helpers_1.nodeSupportsImportAssertions);
            test.suite('node >=16.15.0 <17.0.0 | >=17.5.0', (test) => {
                test.runIf(helpers_1.nodeSupportsImportAssertionsTypeJson);
                test('Can import JSON modules with appropriate assertion', async (t) => {
                    const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ./importJson.ts`, {
                        cwd: (0, path_1.resolve)(helpers_1.TEST_DIR, 'esm-import-assertions'),
                    });
                    expect(err).toBe(null);
                    expect(stdout.trim()).toBe('A fuchsia car has 2 seats and the doors are open.\nDone!');
                });
            });
            test.suite('node <16.15.0 | 17.0.0> <17.5.0', (test) => {
                test.runIf(!helpers_1.nodeSupportsImportAssertionsTypeJson);
                test('Can import JSON using the appropriate flag and assertion', async (t) => {
                    const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} --experimental-json-modules ./importJson.ts`, {
                        cwd: (0, path_1.resolve)(helpers_1.TEST_DIR, 'esm-import-assertions'),
                    });
                    expect(err).toBe(null);
                    expect(stdout.trim()).toBe('A fuchsia car has 2 seats and the doors are open.\nDone!');
        ...
        test.suite('Entrypoint resolution falls back to CommonJS resolver and format', (test) => {
            test('extensionless entrypoint', async (t) => {
                const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ./esm-loader-entrypoint-cjs-fallback/extensionless-entrypoint`);
                expect(err).toBe(null);
                expect(stdout.trim()).toBe('Hello world!');
            });
            test('relies upon CommonJS resolution', async (t) => {
                const { err, stdout } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ./esm-loader-entrypoint-cjs-fallback/relies-upon-cjs-resolution`);
                expect(err).toBe(null);
                expect(stdout.trim()).toBe('Hello world!');
            });
            test('fails as expected when entrypoint does not exist at all', async (t) => {
                const { err, stderr } = await exec(`${helpers_1.CMD_ESM_LOADER_WITHOUT_PROJECT} ./esm-loader-entrypoint-cjs-fallback/does-not-exist`);
                expect(err).toBeDefined();
                expect(stderr).toContain(`Cannot find module `);
            });
        });
        test.suite('spawns child process', async (test) => {
            test.runIf(helpers_1.nodeSupportsSpawningChildProcess);
    ...
    test.suite('node >= 12.x.x', (test) => {
        ...
        test('throws ERR_REQUIRE_ESM when attempting to require() an ESM script when ESM loader is *not* enabled and node version is >= 12', async () => {
            ...
            const { err, stderr } = await exec(`${helpers_1.BIN_PATH} ./index.js`, {
                cwd: (0, path_1.join)(helpers_1.TEST_DIR, './esm-err-require-esm'),
            });
            expect(err).not.toBe(null);
            expect(stderr).toMatch('Error [ERR_REQUIRE_ESM]: Must use import to load ES Module:');
    ...
    test.suite('node < 12.x.x', (test) => {
        ...
        test('Loads as CommonJS when attempting to require() an ESM script when ESM loader is *not* enabled and node version is < 12', async () => {
            ...
            const { err, stdout } = await exec(`${helpers_1.BIN_PATH} ./index.js`, {
                cwd: (0, path_1.join)(helpers_1.TEST_DIR, './esm-err-require-esm'),
            });
            expect(err).toBe(null);
...
test.suite("Catch unexpected changes to node's loader context", (test) => {
    ...
    test('Ensure context passed to loader by node has only expected properties', async (t) => {
        const { stdout, stderr } = await exec(`node --loader ./esm-loader-context/loader.mjs --experimental-json-modules ./esm-loader-context/index.mjs`);
